...
Windows 설치 프로그램은 `.csv`, `.tsv`, `.xlsx`, `.xls`, `.xlsm`, `.xlsb`, `.ods`, `.cell` 형식의 **연결 프로그램** 목록에 시트뷰를 등록합니다. 파일을 더블클릭해서 열려면 파일을 마우스 오른쪽 버튼으로 클릭한 뒤 **연결 프로그램 → 다른 앱 선택 → 시트뷰**를 선택하고, 해당 확장자의 기본 앱으로 쓰려면 **항상**을 선택하세요. 설치 시 기존 기본 앱은 그대로 유지됩니다. 이 방식으로 파일을 열면 시트뷰 창이 새로 열립니다.
