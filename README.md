...
| 한셀 `.cell` | 한셀 2016 파일 3개로 셀 값과 수식 읽기를 확인했습니다. 다른 버전은 형식에 따라 열리지 않을 수 있습니다. |

`.cell`이 열리지 않으면 한셀에서 **파일 → 다른 이름으로 저장 → Excel 통합 문서(.xlsx)** 또는 CSV를 선택한 후 변환된 파일을 열어 주세요. 한컴의 [한셀 도움말](https://help.hancom.com/hoffice/webhelp/9.0/en_us/hcell/file/save_as/save_as.htm)도 이 저장 형식을 안내합니다. 한셀 파일의 전용 글자 서식은 표시하지 않고 셀의 텍스트 값을 읽습니다.

검증에 사용한 사용자 파일은 저장소에 포함하지 않았습니다.
